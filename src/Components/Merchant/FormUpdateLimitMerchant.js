...
class FormUpdateLimitMerchant extends Component {
  ...
  _onChange (e, f) {
    ...
    console.log('_onChange===>', e.target)
    console.log('_onChange===>', e.target.value)
    console.log('_onChange===>', e.target.id)
    onChangeFlag = true
    // const formBody = {
  ...
  _onSubmitForm (e) {
    ...
    submitForm = true
    if (onChangeFlag) this.props.merchantUpdateMinMaxLimit({
    ...
    if (type === 'text' || type === 'number') {
      return (
        <div className='form-group'>
          <label>{label}</label>
          <input type={type} className='form-control' id={id} placeholder={placeholder} onChange={this._onChange} value={parseInt(this.props[id] || 0)} />
        </div>
      )
    }
  }
