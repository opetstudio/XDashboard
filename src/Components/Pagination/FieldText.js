...
export default class FieldText extends Component {
  ...
  render () {
    const { name, id, label, type, placeholder, value, handleOnChange, table, defaultValue, hidden } = this.props
    return (
      <div style={{ display: hidden ? 'none' : 'block' }} className='form-group'>
        ...
        <input isHidden={`${hidden}`} hiddenvalue={hidden} name={name} type={type} className='form-control' id={id} placeholder={placeholder} ref={name} onChange={(e, f) => handleOnChange({ table, field: name, value: e.target.value })} value={value || defaultValue} />
      </div>
    )
  }
}
